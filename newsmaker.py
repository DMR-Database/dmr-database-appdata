...
# Get the highest order number (excluding 99)
def get_highest_order():
    conn = connect_to_db()
    cursor = conn.cursor()
    
    select_query = f"SELECT MAX(CAST(`order` AS UNSIGNED)) FROM {TABLE_NAME} WHERE `order` != 99"
    
    try:
        cursor.execute(select_query)
        result = cursor.fetchone()
        highest_order = result[0] if result[0] is not None else 0
        print(f"Highest order number (excluding 99): {highest_order}")
        return highest_order
    except mysql.connector.Error as err:
        print(f"Failed to retrieve highest order number: {err}")
    finally:
        cursor.close()
        conn.close()
...
# Main function
def main():
    # Get and print the highest order number first
    ...
    # Validate and convert the date if provided, or set to current date if not provided
    formatted_date = validate_date(args.date) if args.date else datetime.now().strftime("%d-%m-%Y")

    # If no order number is given, take the highest order number and increment it by 1
    order_number = args.order if args.order is not None else highest_order + 1

    # Insert news into the database if all necessary args are provided
    if all([order_number, args.category, formatted_date, args.header, args.news]):
        insert_news(order_number, args.urgent, args.shownews, args.category, formatted_date, args.header, args.news)
    else:
